(function ($) {
  ...
  function awdContentSlider() {
    ...
    var slide_timeout, slide_in_timeout;
    var $bg = $("#bg");
    ...
    menu_elem.on("click", function (e) {
      ...
      if (!elem.hasClass("active")) {
        ...
        slide_timeout = setTimeout(function () {
          var goToSlideContent = $(
            ".slide-item[data-slide-id=" + goToSlide + "]"
          );
          if (goToSlideContent) {
    ...
    $form.on("submit", function (e) {
      ...
      if (!awdFormValidation(subscribeEmailVal)) {
        ...
        $.ajax({
          type: "POST",
          url: url,
          data: { email: subscribeEmailVal, emailAddress: awd_subscribeEmail },
          success: function () {
            $subscribeNotice
              .stop(true)
              .hide()
              .addClass("visible")
              .html(awd_subscribeSuccess)
              .fadeIn();
            $submitButton.prop("disabled", false);
            $form[0].reset();
            $subscribeEmail.blur();
  ...
  function awdScrollbar() {
    $(".sections-block").perfectScrollbar({ suppressScrollX: true });
  }
